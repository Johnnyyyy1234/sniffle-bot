	...
	run: async (client, interaction) => {
		try {
			...
			if (userData) {
				...
				const databaseCode = userData.Code ? userData.Code : code;
				const dataWithDatabaseCode = await premiumKeySchema.findOne({
					Code: databaseCode,
				});
				var databaseTime = dataWithDatabaseCode.DateTag;
				var expiryCheck = checkExpiry(dataWithDatabaseCode, interaction, false);

				let msg;
				if (!expiryCheck) {
					msg = `Your product key hasn't expired yet. Would you still like to renew it?\n\nSubscription time ends: <t:${timeformat(
						Number.parseInt(databaseTime),
					)}:R>`;
				} else {
					msg = "Your product key has expired. Would you like to renew it?";
				}
					...
					switch (i.customId) {
						case "renew-yes":
							...
							rEmbed
								...
								.setDescription("Your premium has been renewed!");
							await interaction.editReply({
								embeds: [rEmbed],
								ephemeral: true,
							});
						...
						case "renew-no":
							rEmbed
								.setColor(mConfig.embedColorSuccess)
								.setDescription("Canceling your premium renewal...");
							await interaction.editReply({
								embeds: [rEmbed],
								ephemeral: true,
							});
				...
				const newdata = new userPremiumSchema({
					Code: code,
					UserID: user.id,
				});
				newdata.save();
				...
				rEmbed
					.setColor(mConfig.embedColorSuccess)
					.setDescription(
						`Successfully activated your premium code. Your premium subscription will expire at <t:${timeformat(
							Number.parseInt(formattedTime),
						)}>`,
					);
				await interaction.editReply({ embeds: [rEmbed], ephemeral: true });
...
function checkMaxUsage(data, interaction) {
	if (data.CurrentUsage >= data.Uses) {
		interaction.editReply({
			content:
				"This product key has reached its maximum usage. Please contact the developer if you think this is a mistake.",
		});
		return true;
	}
	return false;
