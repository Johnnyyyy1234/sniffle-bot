          ...
          sub
            ...
            .addIntegerOption((o) =>
              o
                .setName("uses")
                .setDescription(
                  "How many times this code can be redeemed (default: 1, max: 50"
                )
                .setMinValue(1)
                .setMaxValue(50)
            )
          ...
          sub
            ...
            .addStringOption((o) =>
              o
                .setName("expiration")
                .setDescription(
                  "When this code should expire (default: 1 month)."
                )
                .addChoices(
                  { name: "7 Days", value: "7 days" },
                  { name: "1 Month", value: "1 month" },
    ...
    .addSubcommandGroup((sg) =>
      sg
        ...
        .addSubcommand((sub) =>
          sub
            .setName("code")
            .setDescription("Removes a premimum code.")
            .addStringOption((o) =>
              o
                .setName("key")
        ...
        .addSubcommand((sub) =>
          sub
            ...
            .addStringOption((o) =>
              o
                .setName("user-id")
                .setDescription("The user ID whose premium should be removed.")
                .setRequired(true)
            )
        )
    )
        ...
        case "code":
          ...
          if (time === "custom") {
            ...
            expirationCollector = await channel
              ...
              .then((expiration) => {
                if (expiration.first().content.toLowerCase() === "cancel") {
          ...
          switch (subcmd) {
            case "generate":
              ...
              const sEmbed = new EmbedBuilder()
                ...
                .setDescription(
                  "This product key should not be published in public chats. Share this key only with people you trust!"
                )
                .addFields(
                  { name: "Product Key", value: `||${productKey}||` },
                  { name: "Expiration Time", value: `<t:${timeformat(time)}>` },
                  {
                    name: "Allowed user",
                    value: target === "none" ? "none" : `<@${target.id}>`,
                  },
            ...
            case "renew":
              ...
              let schemaData = await premiumKeySchema.findOne({ Code: code });
              if (!schemaData)
                return interaction.editReply({
                  content: "The provided code does not exist in the database.",
                });

              rEmbed
                .setColor(mConfig.embedColorWarning)
                .setDescription("Renewing the premimum code...");
        ...
        case "remove":
          switch (subcmd) {
            ...
            case "user":
              ...
              rEmbed
                .setColor(mConfig.embedColorWarning)
                .setDescription("Removing the premium from the user...");

              interaction.editReply({ embeds: [rEmbed] });

              await userPremiumSchema.findOneAndDelete({ UserID: userId });
