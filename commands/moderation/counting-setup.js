	...
	data: new SlashCommandBuilder()
		...
		.addChannelOption((option) =>
			option
				.setName("channel")
				.setDescription(
					"Please indicate the channel where you would like the counting messages to be sent.",
				)
				.setRequired(true)
		...
		.addIntegerOption((option) =>
			option
				.setName("max-count")
				.setDescription(
					"The maxium number you want the server members to count up to (default 1000)",
				)
				.setRequired(false)
				.setMinValue(1),
		)
		...
		try {
			if (
				...
				return await interaction.reply({
					content:
						"The counting system cannot be set up because you do not have the necessary permissions to do so.",
					ephemeral: true,
				});
			...
			if (existingData) {
				return await interaction.reply({
					content:
						"You already have a counting system in place. To restart it, use the `/counting-disable` command.",
					ephemeral: true,
				});
			}
